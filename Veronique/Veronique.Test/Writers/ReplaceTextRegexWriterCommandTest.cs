...
{
    /// <summary>
    /// Contains tests for the <see cref="ReplaceTextRegexWriterCommand"/> class.
    /// </summary>
    [TestFixture]
    public class ReplaceTextRegexWriterCommandTest
    {
        #region Methods
        ...
        {
            string testFileName = Path.Combine(new FileInfo(System.Reflection.Assembly.GetExecutingAssembly().Location).DirectoryName, "TestFiles", "ReplaceTextRegex.txt");
            IWriterCommand test = WriterCommandManager.CreateByName("replace-text-regex");
            test.Write(new string[] { testFileName, "[0-9]{2,}", "test" });
